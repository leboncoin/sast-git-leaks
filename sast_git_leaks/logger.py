...
def setup_logging(path=None):
    '''
    Setup logging config from yaml file (config/log.yaml)

    Return:
      - False if basic config used because yaml file not loaded
      - True: yaml config file used
    '''
    try:
        with open(path, "rt") as f:
            content = yaml.safe_load(f.read())
        logging.config.dictConfig(content)
        logging.getLogger(__name__).info("Log: yaml config file used")
        return True
    except Exception:
